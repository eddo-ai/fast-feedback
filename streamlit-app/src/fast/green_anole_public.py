...
def load_gsheet_df() -> pd.DataFrame:
    """Load and preprocess local CSV data with column detection.
    Returns every 15 minutes for each user session."""
    csv_path = os.path.join(
        os.path.dirname(__file__), "../../data/green_anole_data_anonymized.csv"
    )
    try:
...
POSTGRES_URI = os.getenv(
    "POSTGRES_URI", "postgresql://user:password@localhost:5432/fast_pilot"
)
...
def load_enriched_data():
    ...
    try:
        ...
        logger.info(f"Successfully loaded {len(df)} records from {POSTGRES_URI}")
        ...
        logger.error(f"Error loading data from database {POSTGRES_URI}: {e}")
...
# Load responses and evaluations from postgres
...
if df_raw is not None:
    ...
    name_col = "name"
    teacher_col = "teacher" if "teacher" in df_raw.columns else None
    hour_col = "hour"
